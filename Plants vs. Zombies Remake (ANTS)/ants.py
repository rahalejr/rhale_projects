...
class Ant(Insect):
    ...
    def __init__(self, health=1, is_buffed=False):
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while not test_place.is_hive():
            if self.min_range <= dist <= self.max_range and test_place.bees:
                return bee_selector(test_place.bees)
            dist += 1
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        """
        # BEGIN Problem 5
        fire_health = self.health - amount
        total_dmg = amount
        if fire_health <=0:
...
# BEGIN Problem 6
class WallAnt(Ant):
    """WallAnt does nothing but blocks all bees. Has high health for a high food cost as well."""
...
# BEGIN Problem 7
class HungryAnt(Ant):
    """HungryAnt eats one random Bee in its place whole, then spends 3 turns chewing it in which
    it cannot do anything. Does nothing if no bees are in its place.
    """
    ...
    def action(self, gamestate):
        """Eats one bee in HungryAnt's place. Spends three turns chewing. Does nothing if no bees."""
        if self.place.bees and not self.chewing:
            b = bee_selector(self.place.bees)
...
class TankAnt(ContainerAnt):
    """TankAnt provides protection to other Ants and damages all bees in its place."""
...
class ScubaThrower(ThrowerAnt):
    """ThrowerAnt that is watersafe, but costs more."""
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    # OVERRIDE CLASS ATTRIBUTES HERE
    # BEGIN Problem EC
    queen_has_arrived = False
    implemented = True   # Change to True to view in the GUI
    # END Problem EC

    def __init__(self, health=1, impostor=False):
        # BEGIN Problem EC
        super().__init__(health)
        self.impostor = impostor
        if self.queen_has_arrived:
            self.impostor = True
        QueenAnt.queen_has_arrived = True
        # END Problem EC
