...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        oprnds = rest.map(lambda x:scheme_eval(x, env))
        validate_procedure(oprt)
        return scheme_apply(oprt, oprnds, env)
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 9
        name, parameters = target.first, target.rest
        body = expressions.rest
        user_procedure = do_lambda_form(Pair(parameters, body), env)
        env.define(name, user_procedure)
        return name
        # END PROBLEM 9
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        nams, vals = bindings.first.first, scheme_eval(bindings.first.rest.first, env)
        names = Pair(nams, names)
